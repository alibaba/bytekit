 ...
 */
package com.alibaba.bytekit.log;
...
/**
 * Expose static methods to get a logger depending on the environment. If SL4J
 * is on the classpath, it will be used. Otherwise, there are two possible
 * fallbacks: Console or {@link java.util.logging.Logger
 * java.util.logging.Logger}). By default, the Console fallback is used. To use
 * the JDK loggers, set the {@value #FALLBACK_PROPERTY}
 ...
 * {@link #useConsoleLoggers()}, {@link #useVerboseConsoleLoggers()},
 * {@link #useJdkLoggers()} and {@link #useSl4jLoggers()} (which may throw an
...
public abstract class Loggers {
    ...
    /**
     * The system property that determines which fallback implementation to use for
     * loggers when SLF4J isn't available. Use {@code JDK} for the JDK-backed
     * logging and anything else for Console-based (the default).
     */
    public static final String FALLBACK_PROPERTY = "bytekit.logging.fallback";
    ...
    /**
     * Attempt to activate the best {@link Logger bytekit Logger} factory, by first
     * attempting to use the SLF4J one, then falling back to either Console logging
     * or {@link java.util.logging.Logger java.util.logging.Logger}). By default,
     * the Console fallback is used. To fallback to the JDK loggers, set the
     * {@value #FALLBACK_PROPERTY} {@link System#setProperty(String, String) System
     * property} to "{@code JDK}".
     *
     * @see #useJdkLoggers()
    ...
    /**
     * Return true if {@link #resetLoggerFactory()} would fallback to
     * java.util.logging rather than console-based logging, as defined by the
     * {@link #FALLBACK_PROPERTY} System property.
     *
    ...
    /**
     * Force the usage of Console-based {@link Logger Loggers}, even if SLF4J is
     * available on the classpath. Console loggers will output
     * {@link Logger#error(String) ERROR} and {@link Logger#warn(String) WARN}
     * levels to {@link System#err} and levels below to {@link System#out}. All
     * levels <strong>except TRACE and DEBUG</strong> are considered enabled. TRACE
     * and DEBUG are omitted.
     * <p>
     * The previously active logger factory is simply replaced without any
     * particular clean-up.
     */
     ...
     * {@link Logger#error(String) ERROR} and {@link Logger#warn(String) WARN}
     * levels to {@link System#err} and levels below to {@link System#out}. All
     * levels (including TRACE and DEBUG) are considered enabled.
     * <p>
     * The previously active logger factory is simply replaced without any
     * particular clean-up.
     */
    ...
    /**
     * Force the usage of JDK-based {@link Logger Loggers}, even if SLF4J is
    ...
    /**
     * Force the usage of SL4J-based {@link Logger Loggers}, throwing an exception
     * if SLF4J isn't available on the classpath. Prefer using
     * {@link #resetLoggerFactory()} as it will fallback in the later case.
     * <p>
     * The previously active logger factory is simply replaced without any
     * particular clean-up.
     */
     ...
     * {@link #resetLoggerFactory()} (or call one of the {@link #useConsoleLoggers()
     * useXxxLoggers} methods).
     *
    ...
    /**
     * Get a {@link Logger}, backed by SLF4J if present on the classpath or falling
     * back to {@link java.util.logging.Logger java.util.logging.Logger}.
     *
    ...
    /**
     * A {@link Logger} that has all levels enabled. error and warn log to
     * System.err while all other levels log to System.out (printstreams can be
     * changed via constructor).
     */
    ...
    private static final class ConsoleLoggerFactory implements LoggerFactory {
        ...
        private static final HashMap<String, Logger> consoleLoggers = new HashMap<String, Logger>();
